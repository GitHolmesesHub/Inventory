module.exports = {
        AddIncomingOrder: (req, res) => {
                ...
                let updateorderquery = "INSERT INTO incomingorder (userID, incomingorderdate) VALUES (1, '" + aincomingorder.date + "')";
                let updateitemquery = "INSERT INTO incomingitems (incomingorderID, productID, warehouseID, incomingitemquantity) VALUES ";
                ...
                db.query(updateorderquery, (err, result) => {
                  ...
                  const quantities = Array.isArray(aincomingorder.quantity) ? aincomingorder.quantity : [aincomingorder.quantity];
              
                  for (let index = 0; index < aincomingorder.productID.length; index++) {
                    let productquery = "UPDATE product SET productquantity = productquantity + " + parseInt(quantities[index]) + " WHERE productID = " + aincomingorder.productID[index];
                  ...
                  db.query(orderquery, (err, result) => {
                    ...
                    for (let i = 0; i < aincomingorder.productID.length; i++) {
                      itemValues.push(`(${orderID}, ${aincomingorder.productID[i]}, ${aincomingorder.warehouseID}, ${quantities[i]})`);
                    }
